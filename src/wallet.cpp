...
CAmount CWallet::GetZerocoinBalance(bool fMatureOnly) const
{
    ...
    return xionTracker->GetBalance(true, false);
}
...
bool CWallet::SelectStakeCoins(std::list<CStakeInput*>& listInputs, CAmount nTargetAmount)
{
    ...
    CAmount nAmountSelected = 0;
    {
        ...
        //Add xION
        CWalletDB walletdb(strWalletFile);
        list<CZerocoinMint> listMints = walletdb.ListMintedCoins(true, true, true, xionTracker);
        vector<CMintMeta> vMints = xionTracker->GetMints(true);
        ...
        for (auto meta : vMints) {
            ...
            if (meta.nHeight < chainActive.Height() - Params().Zerocoin_RequiredStakeDepth()) {
                CXIonStake *input = new CXIonStake(meta.denom, meta.hashSerial);
...
bool CWallet::CreateXIONOutPut(libzerocoin::CoinDenomination denomination, libzerocoin::PrivateCoin& coin, CTxOut& outMint)
{
    ...
    zwalletMain->GenerateDeterministicXION(denomination, coin);
...
bool CWallet::CreateZerocoinMintTransaction(const CAmount nValue, CMutableTransaction& txNew, vector<CZerocoinMint>& vMints, CReserveKey* reservekey, int64_t& nFeeRet, std::string& strFailReason, const CCoinControl* coinControl, const bool isZCSpendChange)
{
    ...
    while (true) {
        ...
        if (!CreateXIONOutPut(denomination, coin, outMint)) {
        ...
        CZerocoinMint mint(denomination, coin.getPublicCoin().getValue(), coin.getRandomness(), coin.getSerialNumber(), false, coin.getVersion(), &privKey);
...
bool CWallet::SpendZerocoin(CAmount nAmount, int nSecurityLevel, CWalletTx& wtxNew, CZerocoinSpendReceipt& receipt, vector<CZerocoinMint>& vMintsSelected, bool fMintChange, bool fMinimizeChange, CBitcoinAddress* addressTo)
{
    ...
    for (CZerocoinMint mint : vMintsSelected) {
        mint.SetUsed(true);
        if (!xionTracker->UpdateMint(mint)) {
...
bool CWallet::SetMintUnspent(const CBigNum& bnSerial)
{
    ...
    mint.SetUsed(false);
    return xionTracker->UpdateMint(mint);
}
