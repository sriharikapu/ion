...
bool CxIONTracker::UnArchive(const uint256& hashPubcoin)
{
...
std::list<CZerocoinMint> CxIONTracker::ListZerocoinMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
...
void CxIONTracker::SetPubcoinUsed(const uint256& hashPubcoin, const bool isUsed)
{
    if (!HasPubcoinHash(hashPubcoin))
        return;;
    CMintMeta meta = GetMetaFromPubcoin(hashPubcoin);
...
std::list<CMintMeta> CxIONTracker::ListMints(bool fUnusedOnly, bool fMatureOnly, bool fUpdateStatus)
{
    CWalletDB walletdb(strWalletFile);
    if (fUpdateStatus) {
        std::list<CZerocoinMint> listMintsDB = walletdb.ListMintedCoins();
        ...
        for (auto& dMint : listDeterministicDB)
            Add(dMint);
        LogPrintf("%s: added %d dxion from DB\n", __func__, listDeterministicDB.size());
