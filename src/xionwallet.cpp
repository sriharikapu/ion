...
CxIONWallet::CxIONWallet(std::string strWalletFile, bool fFirstRun)
{
    this->strWalletFile = strWalletFile;
    CWalletDB walletdb(strWalletFile);

    uint256 seed;
    if (!walletdb.ReadXIONSeed(seed))
        fFirstRun = true;
...
void CxIONWallet::GenerateMintPool()
{
    ...
    for (int i = n; i < nStop; i++) {
        ...
        CKey key;
        SeedToXION(seedZerocoin, bnSerial, bnRandomness, key);
...
void CxIONWallet::SyncWithChain()
{
    ...
    while (found) {
        ...
        for (pair<uint256, uint32_t> pMint : mintPool.List()) {
            ...
            // See if the mint is already known to the xion tracker
            if (xionTracker->HasPubcoinHash(pMint.first))
                continue;
...
void CxIONWallet::SeedToXION(const uint512& seedZerocoin, CBigNum& bnSerial, CBigNum& bnRandomness, CKey& key)
{
...
void CxIONWallet::GenerateDeterministicXION(CoinDenomination denom, PrivateCoin& coin, bool fGenerateOnly)
{
...
void CxIONWallet::GenerateMint(uint32_t nCount, PrivateCoin& coin, CoinDenomination denom)
{
