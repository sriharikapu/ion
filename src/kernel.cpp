...
uint256 stakeHash(unsigned int nTimeTx, CDataStream ss, unsigned int prevoutIndex, uint256 prevoutHash, unsigned int nTimeTxFrom)
{
    //ION will hash in the transaction hash and the index number in order to make sure each hash is unique
...
bool Stake(CStakeInput* stakeInput, unsigned int nBits, unsigned int nTimeBlockFrom, unsigned int& nTimeTx, uint256& hashProofOfStake)
{
    if (nTimeTx < nTimeBlockFrom)
        return error("CheckStakePos2() : nTime violation");
    ...
    //    LogPrintf("CheckStakePos2(): failed to get kernel stake modifier \n");
    //    return false;
    // }
    ...
    for (int i = 0; i < (nHashDrift); i++) //iterate the hashing
    {
        ...
        //hashProofOfStake = stakeHash(nTryTime, ssUniqueID, prevout.n, prevout.hash, nTimeBlockFrom);
...
bool CheckStakePos2(const CDataStream& ssUniqueID, CAmount nValueIn, const uint64_t nStakeModifier, const uint256& bnTarget, unsigned int nTimeBlockFrom, unsigned int& nTimeTx, uint256& hashProofOfStake)
{
    CDataStream ss(SER_GETHASH, 0);
    ss << nStakeModifier;
    hashProofOfStake = Hash(ss.begin(), ss.end());
...
//instead of looping outside and reinitializing variables many times, we will give a nTimeTx and also search interval so that we can do all the hashing here
bool CheckStakePos3(const CDataStream& ssUniqueID, CAmount nValueIn, const uint64_t nStakeModifier, const uint256& bnTarget, unsigned int nTimeBlockFrom, unsigned int& nTimeTx, uint256& hashProofOfStake)
{
    CDataStream ss(SER_GETHASH, 0);
    ss << nStakeModifier << nTimeBlockFrom << ssUniqueID << nTimeTx;
    hashProofOfStake = Hash(ss.begin(), ss.end());
...
bool CheckStake(const CDataStream& ssUniqueID, CAmount nValueIn, const uint64_t nStakeModifier, const uint256& bnTarget, unsigned int nTimeBlockFrom, unsigned int& nTimeTx, uint256& hashProofOfStake)
{
    return (nTimeTx < (unsigned int)Params().DGWStartTime()) ? 
        CheckStakePos2(ssUniqueID, nValueIn, nStakeModifier, bnTarget, nTimeBlockFrom, nTimeTx, hashProofOfStake) :
        CheckStakePos3(ssUniqueID, nValueIn, nStakeModifier, bnTarget, nTimeBlockFrom, nTimeTx, hashProofOfStake);
}
