...
bool CWalletDB::WriteXIONSeed(const uint256& seed)
{
    return Write(string("dzs"), seed);
}

bool CWalletDB::ReadXIONSeed(uint256& seed)
{
...
std::list<CZerocoinMint> CWalletDB::ListMintedCoins(bool fUnusedOnly, bool fMaturedOnly, bool fUpdateStatus, CxIONTracker* xionTracker)
{
    ...
    for (;;)
    {
        ...
        if (xionTracker)
            xionTracker->UpdateMint(mint, false);
    ...
    for (CZerocoinMint& mint : vArchive) {
        xionTracker->Archive(mint, false);
