...
# Get BUILDDIR and REAL_IOND
. "${CURDIR}/tests-config.sh"

export IONCLI=${BUILDDIR}/qa/pull-tester/run-ion-cli
export IOND=${REAL_IOND}
...
if [ "x${ENABLE_IOND}${ENABLE_UTILS}${ENABLE_WALLET}" = "x111" ]; then
  ...
  echo "No rpc tests to run. Wallet, utils, and iond must all be enabled"
fi
